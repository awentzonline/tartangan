...
from tartangan.models.blocks import (
    ...
    IQNGeneratorInputMLP, TiledZGeneratorInput,
    GeneratorOutput, E2EIQNDiscriminatorOutput
)
...
from tartangan.models.pluggan import E2EIQNDiscriminator, Generator, GAN_CONFIGS

from .trainer import Trainer


class EndToEndIQNTrainer(Trainer):
    def build_models(self):
        ...
        d_output_factory = functools.partial(
            E2EIQNDiscriminatorOutput, norm_factory=norm_factory
        )
        self.g = Generator(
            self.gan_config,
    ...
    def train_batch(self, imgs):
        ...
        batch_imgs, labels, taus = self.make_adversarial_batch(imgs)
        ...
        if self.args.grad_penalty:
            real.requires_grad_()
        # TODO: what value should tau take for real images?
        real_taus = torch.ones(len(labels) // 2) * 0.5
        p_labels_real, d_loss_real = self.d(real, real_taus, targets=labels[:len(labels) // 2])
        p_labels_fake, d_loss_fake = self.d(fake.detach(), taus, targets=labels[len(labels) // 2:])
        ...
        batch_imgs, labels, taus = self.make_generator_batch(imgs)
        #torchvision.utils.save_image(batch_imgs, 'batch.png', normalize=True, range=(-1, 1))
        p_labels, g_loss = self.d(batch_imgs, taus, targets=labels)
        g_loss.backward()
    ...
    def make_adversarial_batch(self, real_data, **g_kwargs):
        generated_data, taus = self.sample_g(len(real_data), **g_kwargs)
        ...
        return batch, labels, taus

    def sample_g(self, n=None, target_g=False, **g_kwargs):
        z = self.sample_z(n)
        if target_g:
            imgs, taus = self.target_g(z, **g_kwargs)
        else:
            imgs, taus = self.g(z, **g_kwargs)
        return imgs, taus
...
if __name__ == '__main__':
    ...
    trainer = EndToEndIQNTrainer(args)
    trainer.train()
